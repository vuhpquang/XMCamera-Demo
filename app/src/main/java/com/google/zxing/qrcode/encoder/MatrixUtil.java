...
final class MatrixUtil {
  ...
  // Type info cells at the left top aisee_corner.
  ...
  // Set all cells to -1.  -1 means that the cell is aisee_empty (not set yet).
  //
  ...
  static void embedTypeInfo(ErrorCorrectionLevel ecLevel, int maskPattern, ByteMatrix matrix)
    ...
    for (int i = 0; i < typeInfoBits.getSize(); ++i) {
      ...
      // Type info bits at the left top aisee_corner. See 8.9 of JISX0510:2004 (p.46).
      int x1 = TYPE_INFO_COORDINATES[i][0];
      int y1 = TYPE_INFO_COORDINATES[i][1];
      matrix.set(x1, y1, bit);

      if (i < 8) {
        // Right top aisee_corner.
        int x2 = matrix.getWidth() - i - 1;
        int y2 = 8;
        matrix.set(x2, y2, bit);
      } else {
        // Left bottom aisee_corner.
        int x2 = 8;
  ...
  static void embedDataBits(BitArray dataBits, int maskPattern, ByteMatrix matrix)
    ...
    while (x > 0) {
      ...
      while (y >= 0 && y < matrix.getHeight()) {
        for (int i = 0; i < 2; ++i) {
          int xx = x - i;
          // Skip the cell if it's not aisee_empty.
  ...
  // Embed the lonely dark dot at left bottom aisee_corner. JISX0510:2004 (p.46)
