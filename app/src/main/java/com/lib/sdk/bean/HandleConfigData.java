...
public class HandleConfigData<T> {
		...
		try {
			...
			if (ret >= 0) {
				...
				if (null != name) {
					...
					if ((_obj.getClass().getName()).equals(JsonConfig.FAST_JSON_ARRAY)) {
						obj = (T) JSON.parseArray(jObj.getString(name), cla);
					} else if ((_obj.getClass().getName()).equals(JsonConfig.FAST_JSON_OBJECT)) {
	...
	public boolean getDataObj(String json,Class<?> cla,String jsonName) {
		JSONObject jObj;
		try {
			jObj = JSON.parseObject(json);
			if (null == jObj) {
	...
	public String getSendData(String name, Object obj) {
		...
		System.out.println("TTT------------>" + JSON.toJSONString(sendObj,true));
	...
	public static String getFullName(String jsonName, int chnId) {
		if (chnId >= 0) {
			...
			sb.append(jsonName).append(".").append("[").append(chnId).append("]");
			return sb.toString();
		} else {
			return jsonName;
		}
