...
//import com.example.testproject.utils.XUtils;
...
public class H264_DVR_FILE_DATA implements Serializable {
	...
	public long st_5_wnd = 0;
	public int st_6_StreamType;// 不是获取到的码流类型，以getStreamType()为准

	public int downloadStatus = Define.ON_DOWNLOAD_IN_DOWNLOAD_LIST;
	public int downloadType = Define.MEDIA_TYPE_DEVICE;
	...
	private long getLongTimes(int hour, int minute, int second) {
	...
	public boolean isContain(long times) {
		if (times >= getLongStartTime() && times <= getLongEndTime()) {
			return true;
		} else {
			return false;
		}
